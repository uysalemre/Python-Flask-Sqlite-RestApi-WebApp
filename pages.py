...
def send_password_reset_email(user_email):
    ...
    password_reset_url = url_for('website.reset_with_token', token=password_reset_serializer.dumps(user_email, salt=
    current_app.config['SECURITY_PASSWORD_SALT']), _external=True)
...
def home():
    ...
    return render_template('mainpages/home.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, photo=photo)
...
def apply():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        current_app.db.session.add(
            TempActors(form.name.data, form.surname.data, form.email.data, False, form.gender.data, form.age.data,
                       form.phone.data))
        current_app.db.session.commit()
        actor = TempActors.query.filter_by(email=form.email.data).first()
        current_app.db.session.add(
            TempActorBody(actor.id, form.height.data, form.weight.data, form.bodycolor.data, form.haircolor.data,
                          form.hairtype.data, form.eyecolor.data, form.top.data, form.middle.data, form.bottom.data,
                          form.foot.data))
        current_app.db.session.add(
            TempActorSoul(actor.id, form.facebook.data, form.twitter.data, form.instagram.data, form.description.data))
        ...
        current_app.db.session.add(
            TempPhotos(actor.id, photo1.filename, photo2.filename, photo3.filename, photo4.filename, photo5.filename,
                       photo6.filename, photo7.filename, photo8.filename, photo9.filename, photo10.filename))
        current_app.db.session.commit()
        send_apply_message(form.email.data)
        flash('Application Completed Successfully')
        return redirect(url_for('website.apply'))
    return render_template('mainpages/apply.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, form=form)
...
def contact():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        if last_email:
            if last_email.email == form.email.data and last_email.time + timedelta(
                    seconds=120) >= datetime.datetime.now():
        ...
        if file:
            ...
            current_app.db.session.add(
                NormalContact(form.name.data, form.email.data, form.subject.data, form.message.data, file.filename,
                              datetime.datetime.now()))
        current_app.db.session.add(
            NormalContact(form.name.data, form.email.data, form.subject.data, form.message.data, None,
                          datetime.datetime.now()))
        current_app.db.session.commit()
        flash("Thanks for contact with us")
        send_apply_message(form.email.data,
                           "Your contact message reached us we will make a detailed information to you in next days.")
        return redirect(url_for('website.contact'))
    return render_template('mainpages/contact.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, form=form)
...
def login():
    ...
    return render_template('auth/login.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, form=form)
...
def signup():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        if form.password.data != form.passwordAgain.data:
            ...
            return redirect(url_for('website.signup'))
        current_app.db.session.add(
            User(form.email.data, form.username.data, pwd_content.encrypt(form.password.data), False,
                 datetime.datetime.now()))
        current_app.db.session.commit()
        ...
        flash("Hello {} thanks for signed up after checking your account we will email you about your auth ".format(
            form.username.data))
    return render_template('auth/signup.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, form=form)
...
def profile():
    app, categories, actors, modeling, staticLink = getNavBar()
    return render_template('mainpages/profile.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink)
...
def forgotpassword():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        return redirect(url_for('website.login'))
    return render_template('auth/forgotpassword.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, form=form)
...
def reset_with_token(token):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if girilen == onaylanacak:
            current_app.db.session.query(User).filter(User.email == email).update(
                {User.password: pwd_content.encrypt(girilen)})
            current_app.db.session.commit()
...
def category(category):
    ...
    app, categories, actors, modeling, staticLink = getNavBar()
    photo = Photos.query.all()
    return render_template('mainpages/category.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, existCheck=existCheck, photo=photo)
...
def model(category, id):
    ...
    return render_template('mainpages/model.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, actor=actor, photo=photo, soul=soul, body=body)
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if user:
            if user.has_role('Admin') and pwd_content.verify(passw, user.password):
                ...
                login_user(user)
                current_app.db.session.query(User).filter_by(email=email).update(
...
def aboutus():
    ...
    return render_template('mainpages/aboutus.html', appName=app.appname, categories=categories, actors=actors,
                           modeling=modeling, staticLink=staticLink, agency=agency)
